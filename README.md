...
- **?? Daily Fortune & Entertainment**:
  - `/????`: Daily fortune-telling with daily cooldown lock, lucky color, noble constellation, and custom commentary.
...
- **?? Activity Leveling & Leaderboard**:
  ...
  - `/??`: Personal rank card with visual progress bar (`????`).
  - `/???`: Server Top 30 leaderboard categorised into *Hall of Fame (1–10)*, *Backbone (11–20)*, and *Close Behind (21–30)*.
  - **Admin Commands**: `/????`, `/????`, and `/????` (monthly leaderboard reset with Top 10 honor board broadcast).
...
## ?? Required Discord Bot Privileged Intents
